...
**自比赛数据发放以来，相继接到多个参赛者的反馈，"跨语言短文检索"任务对计算资源需求比较高，经主办方团队研究决定："跨语言短文检索"任务不再作为参赛者最终评分的依据，参赛者可根据自身实际情况自愿选择参加该任务。**
...
### 2、最终排名标准
我们将两个个任务（跨语言话题分类，跨语言短文检索）的分数加权（权重为1/2）求和，得到参赛者最终的比赛得分，得分越高，排名越靠前。
