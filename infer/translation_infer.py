...
def translate(args):
    ...
    idx_by_length = (np.argsort(src_len)
                     if args.decode_by_length else np.arange(len(src_len)))
    candidate = {}
    ...
    print(f"Total sentences: {len(src_sent)}, decoding cost:{end-start:.2f}, \
            speed: {ntokens/(end-start):.3f} ntokens/s")
