...
class BaseTask():
    def __init__(self, data, model, config, vocab, task_id="", local_rank=None):
        self.data = data
        self.model = model
        self.config = config
        self.vocab = vocab
    ...
    def stop_training(self):
        if self.stop_patience >= self.patience:
            logger.info(
                f"Stopping training since no better result occurs in task[{self.task_name}_{self.task_id}]."
            )
            exit()
        elif self.stop_patience != 0:
            logger.info(
                f"No better result occurs in Task_{self.task_name}_{self.task_id}({self.stop_patience}/{self.patience})."
            )
        else:
            logger.info(
    ...
    def save_checkpoint(self, path, name=""):
        ...
        if self.config.get('multi_gpu', False) and dist.get_rank() != 0: return
        state = {
    ...
    def reload_checkpoint(self, path):
        ...
        checkpoint_last_path = os.path.join(
            path, "checkpoint_{}_{}_last".format(
                self.task_name,
                self.task_id,
            ))
        ...
        reload_path = self.config['reload_checkpoint'] if self.config[
            'reload_checkpoint'] else checkpoint_last_path
        ...
        if self.config.get('multi_gpu', False):
            self.model.module.sentenceRep.load_state_dict(
                state['model_sentenceRep'])
            self.model.module.target.load_state_dict(state['model_target'])
        else:
            self.model.sentenceRep.load_state_dict(state['model_sentenceRep'])
