...
from src.utils.utility import to_cuda, accuracy, f1_recall_precision, truncate, distributed_model
...
class ClassificationTask(BaseTask):
        ...
        with torch.no_grad():
            for name in ['valid', 'test']:
                ...
                for i, batch in enumerate(iter(self.data[name])):
                    text, langid, label = batch
                    text = truncate(text, self.max_seq_length,
                                    self.vocab.pad_index, self.vocab.bos_index)
                    golden.extend(label.cpu().tolist())
                    text, langid, label = to_cuda(text, langid, label)
                    logits = self.model(text, langid)
                    scores, predict = logits.topk(k=1, dim=-1)
    ...
    def build_optimizer(self, optimizer=None, share_all_task_model=False):
        ...
        optimizer = Adam(self.model.sentenceRep.parameters() if not multi_gpu else self.model.module.sentenceRep.parameters(),\
                        self.config['lr_e'])
        optimizer = Adam([{
            'params':
    ...
    def build_task(cls,
        ...
        if model:
            ...
            _model = ClassificationModel(config['target'], sentenceRepModel,
                                         vocab)
        else:
            if config['sentenceRep'].get("pretrain_rep", None):
                ...
                sentenceRepModel.load_state_dict(state['model_sentenceRep'])
